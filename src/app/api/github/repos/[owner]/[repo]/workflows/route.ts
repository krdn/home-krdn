/**
 * GitHub Workflows API Route
 * GET /api/github/repos/[owner]/[repo]/workflows - 워크플로우 목록 조회
 *
...
import { getWorkflows } from '@/lib/github-service'
 ...
 * 특정 레포지토리의 GitHub Actions 워크플로우 목록 조회
 *
 * 응답:
 * - 200: { success: true, data: GitHubWorkflow[] }
 * - 401: 인증 필요 또는 토큰 미설정
 * - 404: 레포지토리 없음
 * - 429: Rate limit 초과
 * - 502: GitHub API 에러
...
export async function GET(
  ...
  try {
    ...
    // 3. 워크플로우 목록 조회
    const workflows = await getWorkflows(payload.userId, owner, repo)
    ...
    // 기타 GitHub API 에러
    logger.error('[GitHub] 워크플로우 목록 조회 오류', Logger.errorToContext(error))
    return NextResponse.json(
      {
        success: false,
