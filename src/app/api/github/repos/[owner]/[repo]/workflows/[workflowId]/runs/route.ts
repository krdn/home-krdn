/**
 * GitHub Workflow Runs API Route
 * GET /api/github/repos/[owner]/[repo]/workflows/[workflowId]/runs - 워크플로우 실행 기록 조회
 *
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          success: false,
          error: '레포지토리 또는 워크플로우를 찾을 수 없습니다',
          code: 'NOT_FOUND',
        },
        { status: 404 }
      )
    ...
    // 기타 GitHub API 에러
    logger.error('[GitHub] 워크플로우 실행 기록 조회 오류', Logger.errorToContext(error))
    return NextResponse.json(
      {
        success: false,
