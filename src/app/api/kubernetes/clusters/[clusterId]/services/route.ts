/**
 * Kubernetes Services API Route
 * GET /api/kubernetes/clusters/[clusterId]/services - Service 목록
 *
...
import { getServices } from '@/lib/kubernetes-service';
...
/**
 * GET /api/kubernetes/clusters/[clusterId]/services
 * 클러스터의 Service 목록 조회
 *
 * Query Params:
 ...
 *
 * 응답:
 * - 200: { success: true, data: K8sService[] }
 * - 401: 인증 필요
 * - 404: 클러스터 없음
 * - 502: K8s API 에러
 * - 500: 서버 에러
...
export async function GET(
  ...
  try {
    ...
    // 2. Service 조회
    const services = await getServices(clusterId, payload.userId, {
      namespace,
      labelSelector,
      limit,
    });
    ...
    logger.error('[Kubernetes] Service 조회 오류', Logger.errorToContext(error));
    return NextResponse.json(
      { success: false, error: 'Service 조회에 실패했습니다', code: 'INTERNAL_ERROR' },
      { status: 500 }
    );
  }
}
