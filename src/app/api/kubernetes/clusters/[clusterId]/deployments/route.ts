/**
 * Kubernetes Deployments API Route
 * GET /api/kubernetes/clusters/[clusterId]/deployments - Deployment 목록
 *
...
import { getDeployments } from '@/lib/kubernetes-service';
...
/**
 * GET /api/kubernetes/clusters/[clusterId]/deployments
 * 클러스터의 Deployment 목록 조회
 *
 * Query Params:
 ...
 *
 * 응답:
 * - 200: { success: true, data: K8sDeployment[] }
 * - 401: 인증 필요
 * - 404: 클러스터 없음
 * - 502: K8s API 에러
 * - 500: 서버 에러
...
export async function GET(
  ...
  try {
    ...
    // 2. Deployment 조회
    const deployments = await getDeployments(clusterId, payload.userId, {
      namespace,
      labelSelector,
      limit,
    });
    ...
    logger.error('[Kubernetes] Deployment 조회 오류', Logger.errorToContext(error));
    return NextResponse.json(
      { success: false, error: 'Deployment 조회에 실패했습니다', code: 'INTERNAL_ERROR' },
      { status: 500 }
    );
  }
}
